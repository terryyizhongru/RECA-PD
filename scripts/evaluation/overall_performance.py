...
if __name__ == '__main__':
    ...
    print("\n=== Compact Statistics ===")
    print("Accuracy & F1 (PD) & Precision (PD) & AUC & Sensitivity & Specificity")
    print(f"{np.mean(all_run_accuracies):.4f} ± {np.mean(all_run_accuracies_stds):.4f} & {np.mean(all_run_f1s):.4f} ± {np.mean(all_run_f1s_stds):.4f} & {np.mean(all_run_precisions):.4f} ± {np.mean(all_run_precisions_stds):.4f} & {np.mean(all_run_aucs):.4f} ± {np.mean(all_run_aucs_stds):.4f} & {np.mean(all_run_sensitivities):.4f} ± {np.mean(all_run_sensitivities_stds):.4f} & {np.mean(all_run_specificities):.4f} ± {np.mean(all_run_specificities_stds):.4f}")
